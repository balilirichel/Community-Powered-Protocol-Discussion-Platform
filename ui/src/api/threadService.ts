...
export const threadService = {
  listByProtocol: (protocol: number | string) =>
    apiClient
      .get<PaginatedResponse<Thread>>(`/protocols/${protocol}/threads`)
      .then((r) => r.data),

  get: (protocol: number | string, thread: number | string) =>
    apiClient
      .get<ApiResponse<Thread>>(`/protocols/${protocol}/threads/${thread}`)
      .then((r) => r.data.data),

  create: (protocol: number | string, data: CreateThreadRequest) =>
    apiClient
      .post<ApiResponse<Thread>>(`/protocols/${protocol}/threads`, data)
      .then((r) => r.data.data),

  update: (protocol: number | string, thread: number | string, data: UpdateThreadRequest) =>
    apiClient
      .put<ApiResponse<Thread>>(`/protocols/${protocol}/threads/${thread}`, data)
      .then((r) => r.data.data),

  patch: (protocol: number | string, thread: number | string, data: UpdateThreadRequest) =>
    apiClient
      .patch<ApiResponse<Thread>>(`/protocols/${protocol}/threads/${thread}`, data)
      .then((r) => r.data.data),
  ...
  removeVote: (thread: number | string) =>
    apiClient.delete<void>(`/threads/${thread}/vote`).then((r) => r.data),
};
